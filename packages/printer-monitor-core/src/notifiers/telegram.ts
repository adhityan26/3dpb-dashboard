...
export const escapeCode = (s: string) => s.replace(/([`\\])/g, '\\$1')
...
export class TelegramNotifier implements Notifier {
  ...
  async notify(e: PrinterEvent, ctx: { printerName: string; hmsText: string[] }): Promise<void> {
    ...
    for (const chatId of this.opts.chatIds) {
      const res = await f(`https://api.telegram.org/bot${this.opts.botToken}/sendMessage`, {
        method: 'POST', headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ chat_id: chatId, text, parse_mode: 'MarkdownV2' }),
      })
      if (!res.ok) {
        console.error(`[telegram-notifier] HTTP ${res.status}: ${await res.text().catch(() => '')}`)
      }
    }
  }
}
