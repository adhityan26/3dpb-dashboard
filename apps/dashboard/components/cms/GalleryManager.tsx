  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start justify-between">
        ...
        <button onClick={openCreate} className="px-4 py-2 rounded-[5px] text-[12px] font-semibold text-white" style={{ background: "linear-gradient(135deg,#5055e8,#818cf8)" }}>
          + Tambah
        </button>
      </div>
      ...
      {(creating || editing) && (
        <div className="p-4 rounded-[5px] border space-y-4" style={{ background: "rgba(99,102,241,0.06)", borderColor: "rgba(99,102,241,0.2)" }}>
          ...
          <div>
            ...
            <input className="w-full bg-white/[0.04] border border-white/10 rounded-[5px] px-3 py-2 text-[13px] text-white/80 focus:outline-none" value={form.alt} onChange={(e) => setForm((f) => ({ ...f, alt: e.target.value }))} />
          </div>
          ...
          <div>
            ...
            <select className="w-full bg-white/[0.04] border border-white/10 rounded-[5px] px-3 py-2 text-[13px] text-white/80 focus:outline-none" style={{ background: "rgba(10,10,30,0.8)" }} value={form.category} onChange={(e) => setForm((f) => ({ ...f, category: e.target.value as typeof form.category }))}>
        ...
        renderRow={(item) => (
          <>
            <td className="px-3 py-2 w-[60px]">
              {item.imageUrl && <img src={`${item.imageUrl}?w=48&h=48&fit=crop`} alt={item.alt} className="w-10 h-10 rounded-[5px] object-cover" />}
            </td>
