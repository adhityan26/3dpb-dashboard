...
- Multi-material + katalog filament (Pro, 1b-6a): tiap plate bisa >1 filament. Katalog filament (`LocalSettings.filaments` `{brand,material,tipe,warna,warnaHex?,hpp/g,jual/g}`) dikelola di `/settings` section "Daftar filament" (pola sama `laborJobs`; persist IndexedDB Pro). Di kalkulator tiap plate punya toggle "🎨 Multi-material" (`components/PlateInput.tsx`): baris material `[filament ▾] [berat] [swatch]`, pilih filament auto-isi tipe+warna, `HexColorPicker` (`@3pb/ui`) untuk warna. `buildInputV2` resolve tarif per material dari katalog (`filamentId`) dengan fallback `material[tipe]` → **paritas: material tanpa filamentId = hasil identik sebelum 1b-6a**. Core tak berubah (`materials[]` sudah native). Import `.gcode/.3mf`=1b-6b.
