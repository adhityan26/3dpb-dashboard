...
function toProdukInternalData(raw: any): ProdukInternalData {
  ...
  return {
    ...
    marginTier: k ? (k.marginTier as "A" | "B" | "C" ?? null) : null,
    offlineA: k ? (k.offlineA ?? null) : null,
    offlineB: k ? (k.offlineB ?? null) : null,
    offlineC: k ? (k.offlineC ?? null) : null,
    shopeeA: k ? (k.shopeeA ?? null) : null,
    shopeeB: k ? (k.shopeeB ?? null) : null,
    shopeeC: k ? (k.shopeeC ?? null) : null,
    hargaOfflineAktual: k ? (k.hargaOfflineAktual ?? null) : null,
...
export async function createKatalog(input: ProdukInternalInput): Promise<ProdukInternalData> {
  const row = await prisma.produkInternal.create({
    data: {
      ...
      primaryKalkulasiId: input.primaryKalkulasiId ?? null,
    },
    include: INCLUDE_ALL,
  })
  return toProdukInternalData(row)
